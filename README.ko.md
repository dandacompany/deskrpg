...
- 버전: `v2026.926.1` — 직원마다 MCP 커넥터를 둡니다(추가·연결 테스트·OAuth·다른 직원에게 복사). 무인 실행이 무엇을 해도 되는지 정합니다: 대화·회의 안 승인 카드, 실행 정책, 판단 모음에 모이는 차단된 실행. NPC 응답을 도중에 멈출 수 있습니다. 크론 템플릿·주기·실행 이력이 사용자 언어로 보입니다. 프로젝트 보드를 보관하고 목표일을 정합니다. 실패한 응답은 이유(제공자 로그인 만료·사용 한도·모델)를 알리고, Windows 호스트가 더 튼튼해졌습니다. 아래 안내의 플러그인 0.19.0과 정책 지원 Hermes core가 필요합니다.
