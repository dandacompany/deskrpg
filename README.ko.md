...
- 버전: `v2026.926.2` — 보안 수정: 게이트웨이의 직원 수정·삭제는 이제 그 게이트웨이 소유자만 할 수 있습니다(공유받은 사용자가 토큰·이름을 바꾸거나 모든 오피스에서 직원을 지울 수 있었습니다). 그 밖에 공유·진단이 각자의 페이지로 나뉘고, 칸반 지표가 재작업을 세며, 회의록이 Hermes 시간대를 따르고, 판단 모음은 실패한 크론마다 한 줄만 보이고, `deskrpg db backups` 로 오래된 마이그레이션 백업 표를 확인·정리합니다. 아래 안내의 플러그인 0.20.0과 정책 지원 Hermes core가 필요합니다.
