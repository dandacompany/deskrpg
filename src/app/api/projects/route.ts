...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { errorCode: "failed_to_fetch_projects", error: "Failed to fetch projects" },
      { status: 500 },
    );
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!name)
      return NextResponse.json(
        { errorCode: "project_name_required", error: "name is required" },
        { status: 400 },
      );
    ...
    return NextResponse.json(
      { errorCode: "failed_to_create_project", error: "Failed to create project" },
      { status: 500 },
    );
  }
}
