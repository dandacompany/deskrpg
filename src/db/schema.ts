...
export const characters = pgTable(
  "characters",
  {
    ...
    /** Free-text bio — prepended to every conversation sent to the NPC (spec 2026-09-18). The server enforces a 2,000-char cap. */
    bio: text("bio"),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow(),
  },
...
export const gatewayResources = pgTable(
  "gateway_resources",
  {
    ...
    // pluginStatus follows lastValidationStatus's type (a short enum string, so varchar(40)),
    // and pluginCheckedAt follows lastValidatedAt's type (a timestamp column).
...
/**
 * Records for the pre-execution approval gate (design 2026-09-21 execution-approval-gate).
 *
 * Doesn't violate hard gate 1 — `approval_targets.task_id` only **points at** the Hermes card
 * and doesn't replicate its title, body, or status. Same nature as `channel_kanban_boards`.
 */
export const approvals = pgTable(
  "approvals",
  {
    ...
    channelId: uuid("channel_id")
      .notNull()
      .references(() => channels.id, { onDelete: "cascade" }),
    // "task_execution" | "project_registration" | … — chunk 1 uses this same table with just a different type.
    type: varchar("type", { length: 32 }).notNull(),
    // "pending" | "approved" | "rejected" | "revision_requested" | "cancelled"
    status: varchar("status", { length: 24 }).notNull(),
    /** Requesting profile name (the NPC). Not a user id. */
    requestedBy: varchar("requested_by", { length: 64 }).notNull(),
    /** Locale-independent summary. The screen builds the sentence in the viewer's language. */
    title: text("title").notNull(),
    /** {kind:"meeting"|"manual"|"chat_proposal", id} — the origin that determined approval was needed. */
...
export const approvalTargets = pgTable(
  "approval_targets",
  {
    ...
    /** Hermes card id. Not an FK — Hermes is the source of truth. */
    taskId: varchar("task_id", { length: 64 }).notNull(),
    /** For partial approval. Null follows the decision on the approval as a whole. */
