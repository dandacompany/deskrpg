  ...
  return (
    <div className="bg-surface-raised rounded-lg p-3 border border-border space-y-2">
      <input
        ...
        maxLength={200}
        className="w-full bg-surface text-text text-caption rounded px-2 py-1.5 border border-border focus:outline-none focus:border-primary"
        autoFocus
      />
      <textarea
        value={summary}
        ...
        rows={2}
        className="w-full bg-surface text-text text-[11px] rounded px-2 py-1.5 border border-border focus:outline-none focus:border-primary resize-none"
      />
      ...
      {showAdvanced && (
        <div>
          <input
            ...
            placeholder="0 9 * * *"
            className={`w-full bg-surface text-text text-[11px] font-mono rounded px-2 py-1.5 border focus:outline-none ${
              cronError ? "border-danger" : "border-border focus:border-primary"
            }`}
          />
          {cronError && <p className="text-[10px] text-danger mt-0.5">{cronError}</p>}
          <p className="text-[9px] text-text-dim mt-0.5">{t("schedule.cronHint")}</p>
        </div>
      )}
      ...
      <div className="flex gap-1.5 justify-end">
        <button onClick={onCancel} className="px-2 py-1 text-[11px] text-text-muted hover:text-text rounded">
          <X className="w-3 h-3 inline" />
        </button>
        <button
          onClick={handleSubmit}
          disabled={!title.trim() || !!cronError}
          className="px-2.5 py-1 text-[11px] bg-primary text-white rounded hover:bg-primary/80 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1"
        >
