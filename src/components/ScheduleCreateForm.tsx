  ...
  return (
    <div className="bg-surface-raised rounded-lg p-3 border border-border space-y-2">
      <input
        ...
        maxLength={200}
        className="w-full bg-surface text-text text-[14px] rounded px-2 py-1.5 border border-border focus:outline-none focus:border-primary"
        autoFocus
      />
      <textarea
        value={summary}
        ...
        rows={2}
        className="w-full bg-surface text-text text-[13px] rounded px-2 py-1.5 border border-border focus:outline-none focus:border-primary resize-none"
      />
      ...
      <div className="flex flex-wrap gap-1">
        {PRESETS.map((p) => (
          <button
            ...
            className={`px-2 py-1 rounded text-[13px] transition ${
              cronExpr === p.cron
      ...
      {showAdvanced && (
        <div>
          <input
            ...
            placeholder="0 9 * * *"
            className={`w-full bg-surface text-text text-[13px] font-mono rounded px-2 py-1.5 border focus:outline-none ${
              cronError ? "border-danger" : "border-border focus:border-primary"
            }`}
          />
          {cronError && <p className="text-[12px] text-danger mt-0.5">{cronError}</p>}
          <p className="text-[11px] text-text-dim mt-0.5">{t("schedule.cronHint")}</p>
        </div>
      )}
      ...
      <div className="flex gap-1.5 justify-end">
        <button onClick={onCancel} className="px-2 py-1 text-[13px] text-text-muted hover:text-text rounded">
          <X className="w-4 h-4 inline" />
        </button>
        <button
          onClick={handleSubmit}
          disabled={!title.trim() || !!cronError}
          className="px-2.5 py-1 text-[13px] bg-primary text-white rounded hover:bg-primary/80 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1"
        >
