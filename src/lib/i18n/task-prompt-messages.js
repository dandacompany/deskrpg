const taskPromptMessages = {
  en: {
    ...
    "taskPrompt.coreConfirmInstruction":
      'When you detect a task, ALWAYS confirm first: "{confirmation}" and wait for approval before creating.',
    ...
    "taskPrompt.reminderCreateStep":
      "2. After approval, generate a json:task block in the EXACT format below (no other structure allowed):",
    "taskPrompt.reminderRequiredFields":
      "Required fields: action(create/update/complete/cancel), id, title, status, summary",
    "taskPrompt.reminderAllowedFields":
      "Do not add fields other than these five. Put scope/priority/due details inside the summary text.",
    "taskPrompt.reminderIgnoreCasual":
      "Do not create task blocks for casual conversation or simple questions.",
  },
  ko: {
  ...
  ja: {
    ...
    "taskPrompt.reminderCreateStep":
      "2. 承認後、以下の EXACT 形式で json:task ブロックを生成すること（他の構造は禁止）:",
    "taskPrompt.reminderRequiredFields":
      "必須フィールド: action(create/update/complete/cancel), id, title, status, summary",
    "taskPrompt.reminderAllowedFields":
      "この5項目以外のフィールドを追加しないでください。scope/priority/due などは summary の文章内に含めてください。",
  ...
  zh: {
    ...
    "taskPrompt.reminderCreateStep":
      "2. 获得批准后，按下面的 EXACT 格式生成 json:task 代码块（禁止使用其他结构）：",
    "taskPrompt.reminderRequiredFields":
      "必填字段：action(create/update/complete/cancel), id, title, status, summary",
    "taskPrompt.reminderAllowedFields":
      "不要添加这五个字段以外的内容。scope/priority/due 等信息请写进 summary 文本里。",
