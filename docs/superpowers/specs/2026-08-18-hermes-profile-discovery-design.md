# Hermes 프로필 발견·등록 설계

**상태:** 승인됨 (2026-08-18) · **개정** 2026-08-19 (최종 리뷰 C1 — §4·§5.1·§8·§10)
**선행 스펙:** [2026-08-17-deskrpg-hermes-migration-design.md](2026-08-17-deskrpg-hermes-migration-design.md)
**대상 페이즈:** P1 후속 (P3 이전에 넣을 수 있는 독립 작업)

## 1. 문제

`/gateways`에서 Hermes 게이트웨이를 등록한 뒤, 그 게이트웨이의 프로필을 쓰려면
**프로필 이름과 토큰을 손으로 입력**해야 한다. 게이트웨이 하나에 프로필이 N개
붙는 구조(`hermes_profiles.gateway_id` → `gateway_resources.id`)인데 하나씩
타이핑하는 것은 이 데이터 모델과 맞지 않는다.

오타는 조용히 실패한다. 없는 프로필 이름으로 등록해도 저장은 되고, 그 NPC에게
말을 걸었을 때 비로소 실패한다.

## 2. 조사 결과 — 왜 이 형태인가

설계를 정하기 전에 자동화 가능 경로를 전부 확인했다. 결론이 설계를 결정했으므로
근거를 남긴다.

### 2.1 API Server에는 프로필 열거가 없다

`gateway/platforms/api_server.py`의 상단 독스트링이 정본 라우트 목록(24개)이고,
살아 있는 게이트웨이의 `GET /v1/capabilities` 응답 `endpoints` 맵도 같다.
프로필을 나열하는 엔드포인트는 **없다**.

`GET /v1/models`는 리스너 자신의 프로필 하나만 돌려준다(실측: `{"id": "sophie"}`).
서빙 중인 프로필 목록이 아니다.

### 2.2 대시보드 RPC(`profiles.list`)는 외부 서버에게 닫혀 있다

`tui_gateway/methods_profiles.py`에 `profiles.list` / `create` / `describe` /
`configure`가 있고, Hermes Bot Mode가 이것을 쓴다. 그러나 그 문(`/api/ws`)의
인증은 외부 서버를 상정하지 않는다.

- `hermes_cli/dashboard_auth/token_auth.py` — 머신 베어러 토큰은 **라우트별 옵트인**이고
  ("Only registered paths are token-authable"), 실제로 옵트인한 곳은
  `plugins/dashboard_auth/drain/` **한 곳뿐**이다. `/api/ws`는 포함되지 않는다.
- `hermes_cli/dashboard_auth/ws_tickets.py` — WS 자격증명은 두 형태뿐이다.
  단발 티켓은 `POST /api/auth/ws-ticket`로 받는데 그 자체가 **세션 쿠키 인증**을
  요구한다(사람 로그인 전제). 프로세스 내부 자격증명은 **대시보드가 직접 띄운
  자식**(임베디드 TUI PTY) 전용이며 "never injected into any HTML/SPA"라고 못박혀 있다.

Bot Mode가 이 문을 쓸 수 있는 이유는 **데스크톱 앱 안에서 도는 플러그인**이라
이미 인증된 세션을 물려받기 때문이다. DeskRPG는 제3의 서버이므로 같은 자격을
가질 수 없다.

**따라서 `profiles.*` RPC는 채택하지 않는다.**

### 2.3 남는 두 경로

| 경로 | 로컬 | 원격 | 얻는 것 |
|---|:---:|:---:|---|
| 파일시스템 (`<루트>/profiles/`) | O | X | 이름 + 토큰 |
| 게이트웨이 탐침 (`/p/<이름>/health`) | O | O | 존재 여부 |

실측(멀티플렉스 OFF 시점): `/p/sophie/health` → 200, `/p/nosuch/health` → 404.
**멀티플렉스를 켠 뒤 다시 재면 둘 다 200이다 — §6.1 참조.** 이 표의 "존재 여부"는
멀티플렉스가 꺼진 게이트웨이에서만 얻어진다.

## 3. 결정

| # | 결정 | 근거 |
|---|---|---|
| D1 | 로컬이면 파일시스템으로 목록+토큰 자동, 원격이면 입력 즉시 검증 | 2.3 |
| D2 | 비밀 파일 읽기는 **게이트웨이별 명시적 옵인**, 소유자만 | 아래 5 |
| D3 | 프로필 **바인딩만**. 생성·편집은 범위 밖 | 셸 호출 인젝션 비용 + 원격 불가 |
| D4 | @멘션 피어 대화는 **별도 스펙** | 범위 분리 |
| D5 | `profiles.*` RPC 미채택 | 2.2 |

## 4. 판별 — 언제 "로컬"인가

URL만으로 판단하면 Docker에서 틀린다. DeskRPG가 컨테이너 안이면 `127.0.0.1`은
호스트가 아니라 컨테이너다.

```
1단계  URL 호스트가 루프백인가          127.0.0.1 / localhost / ::1 / 0.0.0.0
2단계  프로필 루트가 실제로 존재하는가   ← 능력 판정
```

2단계가 **능력 검사**다. 컨테이너에는 `~/.hermes`가 없으므로 자동으로 원격 모드로
떨어진다. 별도 분기 없이 스스로 교정된다.

**1단계는 빠른 경로가 아니라 보안 게이트다** (2026-08-19 최종 리뷰 C1 반영).
읽는 대상은 게이트웨이가 아니라 **DeskRPG 호스트의 파일시스템 하나**인데, 발견한
토큰은 이후 그 게이트웨이의 `baseUrl`로 `Authorization: Bearer`로 나간다
(프로필 테스트 버튼 → `validateHermesProfile`). 즉 1단계가 없으면 게이트웨이 URL이
곧 **토큰의 목적지**가 되고, 임의 URL을 등록한 사용자가 호스트의 Hermes 키를 자기
서버로 흘려보낼 수 있다. 1단계가 그 목적지를 루프백으로 못박는다.

구현은 `src/lib/hermes/local-discovery-gate.ts`의 순수 함수
(`isLoopbackHost` / `isLoopbackBaseUrl`)이고, 라우트는 **파일시스템에 손대기 전에**
이것을 통과시킨다 — 루프백이 아닌 게이트웨이는 프로필 루트의 존재 확인조차 하지
않는다. 127.0.0.0/8 전체와 IPv4-mapped IPv6(`::ffff:127.0.0.1`), 대괄호가 붙은
IPv6 호스트, 포트가 붙은 URL이 각각 유닛 테스트로 고정돼 있다.

### 4.1 프로필 루트 해석

Hermes의 규칙을 그대로 따른다 (`hermes_cli/profiles.py:_get_profiles_root`):

```
루트 = (HERMES_HOME이 설정되어 있고 ~/.hermes 밖을 가리키면 그 경로,
        아니면 ~/.hermes)
프로필 디렉토리 = 루트/profiles/
```

주석이 이유를 밝힌다 — "In Docker/custom deployments where HERMES_HOME points
outside ``~/.hermes``, profiles live under ``HERMES_HOME/profiles/`` so they
persist on the mounted volume."

## 5. 옵인

`gateway_resources`에 nullable 두 칼럼을 더한다.

```
local_discovery_opted_in_at   TEXT        언제
local_discovery_opted_in_by   TEXT        누가 (users.id)
```

- 로컬로 확인됐고 아직 옵인 전이면 **"이 머신의 Hermes 프로필 읽기"** 버튼을 노출
- **소유자만** 옵인 가능 (`isOwner`)
- 옵인 이전에는 어떤 파일도 열지 않는다

### 5.1 동의는 두 겹이다 — 레코드 소유자 + 인스턴스 운영자

(2026-08-19 최종 리뷰 C1 반영. 이 절 이전의 설계는 "게이트웨이 레코드의 소유자 ==
머신의 주인"을 가정했는데, DeskRPG는 다중 사용자 가상 오피스이므로 그 가정이 거짓이다.)

`POST /api/gateways`는 로그인만 확인하고 임의의 URL로 게이트웨이 레코드를 만들 수
있게 한다. 그 레코드의 `isOwner`는 **"이 레코드는 내 것"**일 뿐 **"이 머신은 내
것"**이 아니다. 그런데 읽히는 비밀은 머신 운영자의 것이다. 그래서 동의 주체를 둘로
나눈다.

| 게이트 | 누가 켜는가 | 무엇을 막는가 |
|---|---|---|
| `DESKRPG_LOCAL_DISCOVERY_ENABLED` (env, 기본 **꺼짐**) | 인스턴스 **운영자** | 무단 열람 — 운영자가 켜지 않으면 아무도 호스트 파일을 못 읽는다 |
| 게이트웨이 `isOwner` | 레코드 **소유자** | 공유받은 사용자로의 확산 |
| 루프백 검사 (§4 1단계) | — (구조적) | 유출 — 토큰이 임의 목적지로 나가는 것 |

셋을 **모두** 통과해야 파일시스템에 닿는다. 앞의 둘 중 하나라도 아니면 기능은
에러가 아니라 **부재**로 보인다 — GET이 `available: false`를 돌려주고, 프로필 루트가
없는 컨테이너와 화면상 구분되지 않는다. 켜지지 않은 기능에 대해 사용자가 실패
버튼을 누르게 만들지 않기 위해서다. POST는 `403 local_discovery_unavailable`로
거부하며, **옵인 기록 자체도 남기지 않는다** — 나중에 스위치가 켜졌을 때 과거의
동의가 되살아나면 안 된다.

언제·누가를 남기는 이유는 나중에 "앱이 왜 내 토큰을 갖고 있는가"에 답하기
위해서다. 값이 아니라 **동의 사실**을 기록한다.

## 6. 발견

옵인된 로컬 게이트웨이에 대해:

```
프로필 후보   <루트>/profiles/<이름>/ 중 config.yaml 이 있는 것
토큰         <그 디렉토리>/.env 의 API_SERVER_KEY (16자 이상)
             단 default 프로필은 <루트>/.env
제외         이미 이 게이트웨이에 등록된 이름
             PROFILE_NAME_RE 를 통과하지 못하는 이름
             (src/app/api/gateways/[id]/profiles/validation.ts:8)
교차 검증     각 후보를 /p/<이름>/health 로 확인한다. 멀티플렉스 게이트웨이에서는
             모든 이름이 200이므로 실질적으로 거르지 못한다(§6.1). 멀티플렉스가
             꺼진 게이트웨이에서만 유효한 신호다.
```

### 6.1 게이트웨이 탐침은 멀티플렉스에서 아무것도 걸러내지 못한다

**이 절은 실행 검증 뒤 정정됐다.** 원래는 "파일시스템은 이름과 토큰이 있다를,
게이트웨이는 실제로 서빙 중이다를 알려주므로 둘을 겹치면 서로의 약점을 덮는다"고
적었고, 근거로 `/p/nosuch/health` → 404 실측을 인용했다. **그 실측은 유효하지 않다.**

`gateway.multiplex_profiles: true` 인 게이트웨이에서 `/health` 는 **어떤 이름에도
200을 돌려준다.** 실측:

```
/p/sophie/health          → 200
/p/acestep_output/health  → 200
/p/nosuchprofile/health   → 200      ← 존재하지 않는 프로필
```

404 를 관찰했던 시점은 **멀티플렉스를 켜기 전**이었다. `/health` 는 인증이 없어
프로필 해석보다 먼저 응답하는 것으로 보인다. 설정을 바꾼 뒤 그 위에 세운 전제를
다시 재지 않은 것이 원인이다.

**따라서 목록의 신뢰는 `hasToken` 이 진다.** `servedByGateway` 는 남겨 두되
멀티플렉스 게이트웨이에서는 항상 `true` 이므로 아무것도 거르지 않는다. 멀티플렉스가
꺼진 게이트웨이에서는 여전히 유효한 신호이므로 필드와 탐침 자체는 유지한다.

**받아들이는 결과:** `acestep_output` 처럼 프로필과 같은 레이아웃
(config.yaml + .env + SOUL.md)을 가진 비에이전트 디렉토리가 목록에 나타난다.
`.env` 에 `API_SERVER_KEY` 가 없으면 `hasToken: false` 로 선택 불가가 되지만,
키가 우연히 있으면 사용자가 목록에서 골라내야 한다. 등록해도 실동작은 그 프로필로
말을 걸 때 실패할 뿐이므로 피해는 제한적이다.

### 6.2 default 프로필의 예외

실측: `~/.hermes/profiles/default/` 에는 `config.yaml`은 있으나 `.env`가 **없다**.
default 프로필의 홈은 `~/.hermes/` 자체이므로 토큰은 `<루트>/.env`에서 읽는다.
이 예외를 놓치면 default만 조용히 "토큰 없음"으로 표시된다.

### 6.3 화면

체크박스 목록에 프로필당 한 행:

```
[ ] sophie     토큰 있음   게이트웨이 응답 O
[ ] danvi      토큰 있음   게이트웨이 응답 O
[ ] ada        토큰 없음   게이트웨이 응답 O      ← 선택 불가, 사유 표시
[✓] noah       이미 등록됨                        ← 비활성
```

**토큰 값은 어떤 경우에도 표시하지 않는다.** 유무만 보여준다.

선택 후 한 번의 저장으로 `hermes_profiles` 행들을 만든다. 기존 등록 API의 검증을 그대로 통과시킨다 —
`validateProfileRegistration`(이름 규칙 + 토큰 16자 이상, 같은 파일)과
`hermes_profiles_gateway_name_idx` 유니크 제약.

## 7. 원격 검증

프로필 이름 입력란은 `onBlur`(포커스를 벗어날 때) 한 번만 `/p/<이름>/health`를
찌른다. 디바운스가 아니라 `onBlur`를 고른 이유: 사용자가 입력을 끝냈다는 신호가
이미 있는데(포커스 이탈) 굳이 타이머로 흉내 낼 필요가 없고, 타이핑 중 반복
요청이 아예 나가지 않는다.

| 응답 | 상태 | 표시 |
|---|---|---|
| 200 | `ok` | 확인됨 |
| 404 | `not_found` | 이 게이트웨이에 없는 프로필 |
| 무응답·타임아웃 | `unknown` | 확인 불가 |

**이 검증이 실제로 잡는 것은 이름 규칙 위반뿐이다** (§6.1 과 같은 이유). 멀티플렉스
게이트웨이의 `/health` 는 모든 이름에 200 을 주므로, 존재하지 않는 프로필도
`ok` 로 나온다. 실측:

```
sophie          → ok
nosuchprofile   → ok           ← 존재하지 않는데 확인됨
..              → not_found    ← 네트워크에 닿기 전 PROFILE_NAME_RE 가 거부
```

`not_found` 는 사실상 **탐침 전 이름 검증**이 내는 값이고, 404 경로는 멀티플렉스가
꺼진 게이트웨이에서만 살아 있다. 그래도 이 검증을 남기는 이유는 오타로 만든
경로 순회형 이름(`..`, `a/b`)을 입력 시점에 잡아 주기 때문이다.

**제출을 막지 않는다.** 게이트웨이가 일시적으로 죽어 있을 때 등록 자체가
불가능해지면 안 된다 — 실제로 겪은 상황이다(감시자가 하트비트 지연으로
게이트웨이를 30~60초마다 재시작하던 구간).

기존 `probeHermesGateway`(`src/lib/hermes/gateway-probe.ts`)를 프로필 경로까지
받도록 확장해 재사용한다.

## 8. 보안 규칙

- 토큰은 **서버에서만** 읽는다. 클라이언트로 내려보내지 않는다
  (기존 게이트웨이 토큰과 동일한 write-only 규칙)
- 파일에 닿기 전 세 게이트를 모두 통과해야 한다 (§5.1)
  1. 인스턴스 스위치 `DESKRPG_LOCAL_DISCOVERY_ENABLED` (기본 꺼짐)
  2. 게이트웨이 `baseUrl`이 루프백 (§4 1단계) — 토큰의 목적지를 못박는다
  3. 게이트웨이 레코드 소유자이고 그 게이트웨이에 옵인돼 있다
- 옵인 없이는 파일을 열지 않는다 — 라우트 게이트를 하나씩 제거하면 각각 실패하는
  회귀 테스트로 고정한다(§10 기준 6). 상태 코드만이 아니라 **실제 fs 호출**을
  감시한다
- 발견 응답에는 토큰 값이 포함되지 않는다 — 유무 불리언만
- 저장 시 기존 경로와 같이 암호화해 넣는다 (`v1:iv:tag:암호문`)

## 9. 범위 밖

| 항목 | 사유 |
|---|---|
| 프로필 생성·편집 | 셸 호출 인자 인젝션 방어가 필수 비용이고, 원격에서 동작하지 않아 기능이 반쪽이 된다 |
| @멘션 피어 대화 | `ConversationEngine`(P2) 위에 올리는 별도 스펙 |
| `profiles.*` RPC | 2.2 — 외부 서버용 인증 경로 없음 |
| 게이트웨이 레벨 토큰 정리 | Hermes에서는 사실상 미사용(인증이 프로필별)이나, 제거는 OpenClaw 어댑터 제거와 함께 간다 |

## 10. 완료 기준

1. 로컬 게이트웨이에서 옵인 후 프로필 목록이 뜨고, 체크 후 한 번의 저장으로
   등록된다. 토큰을 한 번도 입력하지 않는다.
2. **(정정됨 — §6.1)** 멀티플렉스 게이트웨이에서는 `acestep_output` 같은
   비에이전트 디렉토리도 목록에 나타난다. 걸러 주는 것은 `hasToken` 뿐이다.
   원래 기준은 "게이트웨이 응답 없음으로 구분되어 선택되지 않는다"였으나,
   `/health` 가 모든 이름에 200 을 주므로 성립하지 않는다.
3. `default` 프로필이 `<루트>/.env`에서 토큰을 찾아 "토큰 있음"으로 나온다.
4. 컨테이너(또는 프로필 루트가 없는 환경, 또는 인스턴스 스위치가 꺼진 환경,
   또는 루프백이 아닌 게이트웨이)에서는 옵인 버튼이 나타나지 않고, 원격 검증만
   동작한다. 네 경우가 화면에서 구분되지 않는다 — 전부 `available: false`다.
5. **(정정됨 — §7)** 이름 입력 시 규칙 위반(`..`, `a/b`)은 네트워크에 닿기 전
   `not_found` 로 표시된다. 존재하지 않는 프로필은 멀티플렉스 게이트웨이에서
   `ok` 로 나온다. 게이트웨이가 죽어 있어도 **저장은 가능해야 한다**(이 부분은 유효).
6. 옵인하지 않은 게이트웨이에서는 서버가 파일시스템을 건드리지 않는다
   (테스트로 고정). "고정"의 기준은 **게이트를 지우면 테스트가 빨개진다**이다 —
   옵인 게이트(GET/POST), 소유자 게이트(GET/POST), 루프백 게이트, 인스턴스 스위치
   각각에 대해 변이 실험으로 확인한다.
7. 어떤 API 응답에도 프로필 토큰 값이 포함되지 않는다 (테스트로 고정).
